...
public class NIOSocketServer
{
    ...
    public NIOSocketServer(int port, final Dispatcher dispatcher)
    {
    ...
    private DataTransferPacket receive(AsynchronousSocketChannel channel)
    {
        ...
        try{
            int bytesRead = channel.read(byteBuffer).get(5, TimeUnit.SECONDS);
            ...
            while(bytesRead != -1 && running){
                ...
                if(byteBuffer.position() > 2){
                    ...
                    bytesRead = channel.read(byteBuffer).get(2, TimeUnit.SECONDS);
                } else {
                    running = false;
                }
            }
